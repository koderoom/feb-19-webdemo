...
function likeCount(refElement) {
    console.log("LIKE COUNT");
    console.log(refElement);

    // ACCESS THE ELEMENT
    let countElement = refElement.children[1];

    // ACCESS VALUE
    let countElementValue = countElement.innerHTML;

    // CONVERT THE VALUE FROM STRING TO NUMBER
    let countElementValueAsNumber = parseInt(countElementValue);

    countElementValueAsNumber += 1;

    //UPDATE THE ELEMENT
    countElement.innerHTML = countElementValueAsNumber;
